...
public class PlayerLightAttackingState : PlayerGroundedState    
{
    ...
    {
        Vector3 thirdDDir = CameraManager.instance.GetTargetDirection(Player.instance.CameraLookP.transform.localToWorldMatrix.GetPosition(),
        Player.instance.transform.position);
        Vector2 TargetDirection = new Vector2(thirdDDir.x, thirdDDir.z);
        
        if (CameraManager.instance.EnemyLookPoint != null)
        {
            thirdDDir = CameraManager.instance.GetTargetDirection(CameraManager.instance.EnemyLookPoint.transform.localToWorldMatrix.GetPosition(),
    Player.instance.transform.position);
            TargetDirection = new Vector2(thirdDDir.x, thirdDDir.z);
        }
    ...
    protected override void OnDashStated(InputAction.CallbackContext context)
    {
